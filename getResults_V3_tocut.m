function getResults_V3_tocut(queryFolder,waveFolder,ext,resultDirname)
...
%                 Et=(FS/100).*(s2(2));
                ...
                outFile=[queryFolder '/' resultDirectory '/' resultDirname '/' resultFiles(t).name '/' outF];
                %wavwrite(Y(St:Et),44100,outFile)
                outLine=[waveForm ' ' outFile ' ' num2str(stTime) ' ' num2str(etTime)];
                dlmwrite('~/Data/RLCS_motif_match/Results_RLCS_Group/Kamboji/toCut.txt',outLine,'delimiter','','-append')
            end
